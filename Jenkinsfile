pipeline {
  ...
  -Dsonar.host.url=http://192.168.112.244:9000 \
  ...
  -Dsonar.java.binaries=src/main "
        ...
        stage("nexus") {
        steps{
           sh 'mvn deploy:deploy-file -DgroupId=tn.esprit.rh -DartifactId=achat -Dversion=1.0 -DgeneratePom=true -Dpackaging=jar -DrepositoryId=deploymentRepo -Durl=http://192.168.112.244:8081/repository/maven-releases/  -Dfile=target/achat-1.0.jar'
        }
